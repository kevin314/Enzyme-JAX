...
extern "C" std::string runLLVMToMLIRRoundTrip(std::string input,
 ...
 // Add print pass to capture MLIR after blas-raise
 pass_pipeline += "print{filename=/mnt/fast-disks/hao_lab/kevin/blas_raise_output.mlir},";
 if (StringRef(backend).starts_with("xla")) {
   // The blas-raise pass runs before raise-affine-to-stablehlo at the LLVM level
   // where we can still see cublasSgemm calls and their shape parameters (m, n, k)
     // lower-enzymexla-blas pass expands BLAS operations into affine loops
     // so that raise-affine-to-stablehlo can process them
     pass_pipeline += "lower-enzymexla-blas{backend=";
     pass_pipeline += backend;
     pass_pipeline += "},";
     // Debug: capture MLIR after BLAS lowering
     pass_pipeline += "print{filename=/mnt/fast-disks/hao_lab/kevin/after_lower_blas.mlir},";
     ...
     pass_pipeline += backend;
     pass_pipeline += "}";
     // Add print pass to capture MLIR after XLA backend lowering
     pass_pipeline += ",print{filename=/mnt/fast-disks/hao_lab/kevin/blas_raise_output_xla_final.mlir}";
 } else {
     if (outfile.size() && getenv("EXPORT_REACTANT")) {
       pass_pipeline += "print{filename="+outfile+".mlir},";
     }
